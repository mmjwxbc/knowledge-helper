...
"""
20260320腾讯AI应用开发一面
1.介绍auto deployer的实现难点
2.介绍motion graphic agent是什么, 架构是什么
3.项目用的是把skill直接塞进系统提示词中,如果skill太多, 占用的上下文窗口太大, 怎么处理?
4.如果两个skill 实际内容不一样, 但是description相似, 导致agent每次加载错skill, 怎么解决?
5.agent设计你觉得最重要的部分 上下文工程
6.上下文工程你觉得有哪些要注意的点?
7.了解主流的agent设计吗?
8.gemini cli设计你觉得最重要的部分
9.gemini cli 你觉得可以优化的点
10.multi agent 架构主流的实现方案
11.multi agent的交流是靠什么
12手撕 判断有效括号
...
if __name__ == "__main__":
    ...
    # 1. 初始化模型（这里以 OpenAI 为例，你可以换成任意支持流式的模型）
    model = ChatDeepSeek(
        model="deepseek-chat",  # 或 DeepSeek/GLM 等
        temperature=0.3,      # 分类用低温度保证稳定
        api_key="sk-d1fca1676b624c6cb783278cbe833673",
    ...
    # 2. 创建一个最简单的 Agent（不带工具，方便演示）
    agent = create_agent(model, tools=[])

    inputs = {"messages": [HumanMessage(content="讲一个关于代码的冷笑话，50字以内")]}

    print("--- 开始演示三种流式模式 ---")
    
    print("\n🔥 [Mode: values] - 节点运行完，返回当前所有信息:")
    for chunk in agent.stream(inputs, stream_mode="values"):
        # 打印最后一条消息
        msg = chunk["messages"][-1]
        print(f"[{msg.__class__.__name__}]: {msg.content[:20]}...")
        
    print("\n✨ [Mode: updates] - 节点运行完，仅返回该节点的增量:")
    for chunk in agent.stream(inputs, stream_mode="updates"):
        for node_name, data in chunk.items():
            print(f"节点 [{node_name}] 处理完毕，更新了数据。")
            # 这里的 data 只有当前节点的输出
            
    print("\n🌊 [Mode: messages] - 真正的流式，逐 Token 返回:")
    for msg, metadata in agent.stream(inputs, stream_mode="messages"):
        # 这里的 msg 是 AIMessageChunk
        if msg.content:
